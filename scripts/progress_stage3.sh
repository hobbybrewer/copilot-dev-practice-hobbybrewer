...
if [ "$PROGRESS_VERBOSE" = true ]; then 
  echo "TOKENS"
  env | grep TOKEN >> ./scripts/progress_stage3.txt
fi
...
if [ "$PROGRESS_VERBOSE" = true ]; then echo "$GRAPHQL"; fi && GRAPHQL=$(echo "$GRAPHQL" | sed -e 's/SELECTION_ID/'"$SELECTION_ID4"'/g') && if [ "$PROGRESS_VERBOSE" = true ]; then echo "$GRAPHQL"; fi
