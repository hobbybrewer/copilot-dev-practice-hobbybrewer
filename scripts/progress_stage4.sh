...
if [ "$PROGRESS_VERBOSE" = true ]; then 
  echo "TOKENS"
  env | grep TOKEN >> ./scripts/progress_stage4.txt
fi
...
if [ "$PROGRESS_VERBOSE" = true ]; then echo "$GRAPHQL"; fi && GRAPHQL=$(echo "$GRAPHQL" | sed -e 's/SELECTION_ID/'"$SELECTION_ID5"'/g') && if [ "$PROGRESS_VERBOSE" = true ]; then echo "$GRAPHQL"; fi
